// Blocksmith Game with Complete Database Integration
// This version uses Firebase for all user data, authentication, and admin functions

import { blocksmithDB } from './firebase-database.js';

class BlocksmithGameWithDatabase {
    constructor() {
    ...
    async init() {
        console.log('Initializing Blocksmith Game with Database...');
        ...
        try {
            ...
            this.subscribeToRealTimeLeaderboard();
    ...
    async waitForAuthState() {
        return new Promise((resolve) => {
            const checkAuth = () => {
                if (blocksmithDB.currentUser !== undefined) {
                    this.currentUser = blocksmithDB.currentUser;
                    if (this.currentUser) {
                        this.updateUI();
    ...
    async handleLogin(e) {
        ...
        try {
            const result = await blocksmithDB.loginUser(email, password);
    ...
    async handleRegister(e) {
        ...
        try {
            const result = await blocksmithDB.registerUser(username, email, password);
            ...
            if (result.success) {
                ...
                // Show special message for first user (owner)
                if (result.user.rank === 'owner') {
                    this.showStatus('🎉 You are the first user! You have been granted Owner privileges!', 'success');
                }
            } else {
                this.showStatus(result.error, 'error');
            }
    ...
    async handlePasswordChange(e) {
        ...
        try {
            const result = await blocksmithDB.changePassword(currentPassword, newPassword);
            ...
            this.showStatus('Password change failed. Please try again.', 'error');
        } finally {
            this.hideLoading();
        }
    }
    ...
    async cashOut() {
        ...
        await blocksmithDB.updateUserBalance(this.currentUser.uid, newBalance);
        this.currentUser.balance = newBalance;
        
        // Save game result
        await blocksmithDB.saveGameResult(this.currentUser.uid, {
            bet: this.currentBet,
    ...
    async updateLeaderboard() {
        try {
            const leaderboard = await blocksmithDB.getLeaderboard(10);
    ...
    subscribeToRealTimeLeaderboard() {
        this.leaderboardUnsubscribe = blocksmithDB.subscribeToLeaderboard((leaderboard) => {
            const leaderboardList = document.getElementById('leaderboardList');
            leaderboardList.innerHTML = '';
    ...
    async loadGameHistory() {
        ...
        try {
            const games = await blocksmithDB.getGameHistory(this.currentUser.uid, 10);
    ...
    // Admin Commands with Database
    async executeAdminCommand() {
        if (!this.currentUser) {
            this.logAdminCommand('Please log in to use admin commands');
            return;
        }
        ...
        if (cmd === '.give' && parts.length >= 3) {
            ...
            const result = await blocksmithDB.adminGiveMoney(player, amount, this.currentUser.uid);
            if (result.success) {
            ...
            const result = await blocksmithDB.adminRemoveMoney(player, amount, this.currentUser.uid);
            if (result.success) {
                this.logAdminCommand(`✓ Removed $${this.formatNumber(amount)} from ${player}. New balance: $${this.formatNumber(result.newBalance)}`);
            } else {
                this.logAdminCommand(`Error: ${result.error}`);
            }
        } else if (cmd === '.rank' && parts.length >= 3) {
            const player = parts[1];
            const newRank = parts[2].toLowerCase();
            
            if (!['user', 'elite', 'admin', 'owner'].includes(newRank)) {
                this.logAdminCommand('Invalid rank. Use: user, elite, admin, or owner');
                return;
            }
            
            const result = await blocksmithDB.adminChangeRank(player, newRank, this.currentUser.uid);
            if (result.success) {
                this.logAdminCommand(`✓ Changed ${player}'s rank to ${newRank.toUpperCase()}`);
            } else {
                this.logAdminCommand(`Error: ${result.error}`);
            }
            ...
            const result = await blocksmithDB.adminBanUser(player, this.currentUser.uid);
            if (result.success) {
                this.logAdminCommand(`✓ Banned user ${player}`);
            } else {
                this.logAdminCommand(`Error: ${result.error}`);
            }
        } else {
            ...
            this.logAdminCommand(`.rank player rank - Change player rank (user/elite/admin/owner)`);
            this.logAdminCommand(`.ban player - Ban a player`);
        }
    ...
    handleBalanceClick() {
        ...
        const hasAdminAccess = this.currentUser.rank === 'owner' || 
                              this.currentUser.rank === 'admin' || 
                              (this.currentUser.adminPermissions && this.currentUser.adminPermissions.length > 0);
...
// Export for use
export { BlocksmithGameWithDatabase };
